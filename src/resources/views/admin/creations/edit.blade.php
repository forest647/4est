...
@section('content')
    <h1 class="text-2xl font-bold mb-6">Edit Creation #{{ $creation->id }}</h1>

    <form method="POST" action="{{ route('admin.creations.update', $creation) }}" enctype="multipart/form-data" class="max-w-3xl space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="size" id="size" value="{{ old('size', $creation->size) }}"
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price" id="price" value="{{ old('price', $creation->price) }}"
                       class="w-full bg-slate-700 border border-slate-600 rounded px-3 py-2 text-white focus:border-[#39792D] focus:outline-none">
            ...
            <div>
                ...
                <select name="category_id" id="category_id"
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ old('category_id', $creation->category_id) == $category->id ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="material_id" id="material_id"
                    ...
                    @foreach($materials as $material)
                        <option value="{{ $material->id }}" {{ old('material_id', $creation->material_id) == $material->id ? 'selected' : '' }}>
                            {{ $material->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            @if($creation->download)
                <p class="text-sm text-slate-400 mb-1">Current: {{ $creation->download }}</p>
            @endif
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="translations[en][name]" id="translations_en_name"
                       value="{{ old('translations.en.name', $translations['en']->name ?? '') }}"
                       class="w-full bg-slate-700 border border-slate-600 rounded px-3 py-2 text-white focus:border-[#39792D] focus:outline-none">
            ...
            <div>
                ...
                <textarea name="translations[en][description]" id="translations_en_description" rows="4"
                          class="w-full bg-slate-700 border border-slate-600 rounded px-3 py-2 text-white focus:border-[#39792D] focus:outline-none">{{ old('translations.en.description', $translations['en']->description ?? '') }}</textarea>
            ...
            <div>
                ...
                <input type="text" name="translations[ro][name]" id="translations_ro_name"
                       value="{{ old('translations.ro.name', $translations['ro']->name ?? '') }}"
                       class="w-full bg-slate-700 border border-slate-600 rounded px-3 py-2 text-white focus:border-[#39792D] focus:outline-none">
            ...
            <div>
                ...
                <textarea name="translations[ro][description]" id="translations_ro_description" rows="4"
                          class="w-full bg-slate-700 border border-slate-600 rounded px-3 py-2 text-white focus:border-[#39792D] focus:outline-none">{{ old('translations.ro.description', $translations['ro']->description ?? '') }}</textarea>
                @error('translations.ro.description') <p class="text-red-400 text-sm mt-1">{{ $message }}</p> @enderror
            </div>
        </div>

        {{-- Existing Gallery Images --}}
        @if($creation->galleryImages->count())
            <div>
                <h2 class="text-lg font-semibold border-b border-slate-700 pb-2 mb-4">Current Images</h2>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @foreach($creation->galleryImages as $image)
                        <div class="relative bg-slate-800 border border-slate-700 rounded overflow-hidden">
                            <img src="{{ asset('storage/creations/' . str_pad($creation->id, 8, '0', STR_PAD_LEFT) . '/' . $image->filename) }}"
                                 alt="{{ $image->filename }}" class="w-full h-32 object-cover">
                            <label class="flex items-center gap-2 p-2 text-sm text-red-400 cursor-pointer">
        ...
        {{-- New Images --}}
        <div>
            <label for="images" class="block text-sm text-slate-300 mb-1">Add More Images</label>
